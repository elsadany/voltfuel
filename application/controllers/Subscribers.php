...
class Subscribers extends Admin_Controller
{
    ...
    public $cmp_setting_detail = array();
    ...
    public function __construct()
    {
        ...
        $this->cmp_setting_detail = $this->setting_model->getSetting();
        //$this->role;
    }

    public function index()
    {
        //$this->session->set_userdata('top_menu', 'Subscribers');
        $this->session->set_userdata('sub_menu', 'Subscribers/subscriberslist');
        $data['title']       = 'Subscribers List';
        $subscribers_result      = $this->subscribers_model->getsubscribers();
        $data['subscriberslist'] = $subscribers_result;
        $this->load->view('layout/header', $data);
        $this->load->view('subscribers/subscribers_list', $data);
        $this->load->view('layout/footer', $data);
    }

    
    public function makesubscriberactive()
    {
        $id=$this->input->post('id');
        $data['success']=$this->subscribers_model->makesubscriberactive($id);
        echo json_encode($data);
    }

    public function makesubscriberinactive()
    {
        $id=$this->input->post('id');
        $data['success']=$this->subscribers_model->makesubscriberinactive($id);
        echo json_encode($data);
    }

    public function installs(){
        //$this->session->set_userdata('top_menu', 'Subscribers');
        $this->session->set_userdata('sub_menu', 'Subscribers/installations');
        $data['title']       = 'Installations List';
        $installations_result      = $this->subscribers_model->getinstallations();
        $data['installationslist'] = $installations_result;
        $this->load->view('layout/header', $data);
        $this->load->view('subscribers/installations_list', $data);
        $this->load->view('layout/footer', $data);
    }
